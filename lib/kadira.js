...
Kadira.connect = function(appId, appSecret, options) {
  ...
  options.clientEndpoint = options.clientEndpoint || "https://enginex.kadira.io";
  options.serverEndpoint = options.serverEndpoint || "https://enginex.kadira.io";
  ...
  if(_.last(options.clientEndpoint) === '/') {
    options.clientEndpoint = options.clientEndpoint.substr(0, options.clientEndpoint.length - 1);
  }

  if(_.last(options.serverEndpoint) === '/') {
    options.serverEndpoint = options.serverEndpoint.substr(0, options.serverEndpoint.length - 1);
  }
  ...
  if (Meteor.isServer) {
    console.log("Server sync date");
    Kadira.syncedDate = new Ntp(options.serverEndpoint);
    Kadira.syncedDate.sync();
    Kadira.models.error = new ErrorModel(appId);
  } else {
    console.log("Client sync date");
    Kadira.syncedDate = new Ntp(options.clientEndpoint);
    Kadira.syncedDate.sync();
    Kadira.models.error = new ErrorModel(appId);
  }
  ...
  if (Meteor.isServer) {
    console.log("Server runtime info");
    ...
    console.log("Client runtime info");
    __meteor_runtime_config__.kadira = {
      appId: appId,
      clientEndpoint: options.clientEndpoint,
      clientEngineSyncDelay: options.clientEngineSyncDelay,
    };
  }
