Kadira.send = function (payload, path, callback) {
  ...
  var clientEndpoint = Kadira.options.clientEndpoint + path;
  var serverEndpoint = Kadira.options.serverEndpoint + path;
  var retryCount = 0;
  var retry = new Retry({
    minCount: 1,
    minTimeout: 0,
...
Kadira._clientSend = function (clientEndpoint, payload, callback) {
  fetch(clientEndpoint, {
    method: 'POST',
    headers: {
...
Kadira._serverSend = function (serverEndpoint, payload, callback) {
  ...
  new Fiber(function() {
    ...
    HTTP.call('POST', serverEndpoint, httpOptions, function(err, res) {
      if(res) {
